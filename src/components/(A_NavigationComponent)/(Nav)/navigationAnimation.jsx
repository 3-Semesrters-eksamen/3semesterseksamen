...
function topLetterVariants(isActive) {
  return {
    rest: {
      y: isActive ? "-100%" : "0%",
      color: isActive ? "text-nightclub-pink" : "text-white",
      transition: { duration: DURATION, ease: EASE },
    },
    hover: {
      y: "-100%",
      color: "text-nightclub-pink",
      transition: { duration: DURATION, ease: EASE },
    },
  };
}
  ...
  return (
    <Link href={href}>
      <motion.div className="relative overflow-hidden cursor-pointer" initial="rest" whileHover="hover" animate="rest" onMouseEnter={(e) => onMouseEnter(e.currentTarget)}>
        ...
        <SplitText label={label} letterVariants={bottomLetterVariants(isActive)} className="absolute inset-0 text-nightclub-pink text-sm tracking-widest flex" />
      </motion.div>
    </Link>
  );
}
