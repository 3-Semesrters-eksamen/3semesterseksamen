...
const inputBase = "bg-transparent border border-white/20 text-white placeholder-[#555] text-[13px]font-sans px-4 py-3 outline-none w-full transition-colors focus:border-white/60";
const inputErr = "bg-transparent border border-red-600 text-white placeholder-[#555] text-[13px] font-sans px-4 py-3 outline-none w-full";
...
export default function ReserveForm({ selectedTable, eventId, eventDate, onTableConflict, onDatePick, apiUrl }) {
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 px-4">
          <div className="bg-[#111] border border-[#2a2a2a] rounded-lg p-10 max-w-sm w-full flex flex-col items-center text-center shadow-2xl">
            ...
            <button
              ...
              className="border border-[#555] text-[#ccc]  text-[11px] tracking-[0.25em] px-8 py-3 uppercase hover:border-nightclub-pink hover:text-nightclub-pink transition-colors"
            >
              CLOSE
            </button>
          </div>
      ...
      {rootError && <div className="mb-5 px-4 py-3 border border-red-800 bg-red-900/20 rounded text-red-400 text-[12px] ">✗ {rootError}</div>}
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6" noValidate>
        ...
        {/* Table */}
        <div className="flex flex-col gap-1">
          <input type="number" placeholder="Table Number" value={fields.table} readOnly={!!selectedTable} onChange={set("table")} className={`${errors.table ? inputErr : inputBase} ${selectedTable ? "text-[#c9a84c] cursor-default" : ""}`} />
          {errors.table && (
            <span className="text-red-400 text-[11px] " role="alert">
              {errors.table}
            </span>
        ...
        {!eventDate ? (
          ...
          <div className="flex flex-col gap-1">
            <input type="text" readOnly value={new Date(eventDate).toLocaleDateString("en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric" })} className={`${inputBase} text-[#c9a84c] cursor-default`} />
          </div>
        )}
